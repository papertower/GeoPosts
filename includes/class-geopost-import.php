...
class GeoPostImport
{
    ...
    public static function load()
    {
        ...
        add_filter('piklist_pre_update_option_geopost-settings', [__CLASS__, 'pre_update_option'], 10, 3);
    ...
    private static function set_notification($type, $message)
    {
        ...
        update_option(self::NOTICE_SETTING, [
            'type'    => $type,
            'message' => $message
        ]);
    }
    ...
    public static function get_keys($type, $required = true)
    {
        switch (strtolower($type)) {
            case 'post':
                $keys = $required
                    ? ['post_title']
                    : ['ID', 'post_name', 'post_status', 'post_author', 'post_date', 'post_date_gmt'];
                break;

            case 'meta':
                $keys = $required ? ['street', 'city', 'state', 'zip', 'country'] : [];
                break;
    ...
    public static function pre_update_option($settings, $new, $old)
    {
        ...
        if ( ! array_key_exists('csv', $mime_types)) {
            self::set_notification('error',
                'Error: CSV file type not permitted. Please contact your site administrator');
    ...
    private static function import_post_from_data($contents, $header, $options)
    {
        ...
        if ( ! empty($missing_keys)) {
            return new WP_Error ('import_error',
                'The following required columns were missing: ' . implode(', ', $missing_keys));
        }
        ...
        foreach ($contents as $index => $data) {
            ...
            foreach ($post_keys as $key) {
                if (isset($key_indexes[$key])) {
                    switch ($key) {
                        ...
                        default:
                            $post[$key] = apply_filters('geopost_import_post_data', $data[$key_indexes[$key]], $key,
                                $data);
                    }
                }
            }
            ...
            switch (true) {
                ...
                case ($coordinates_included && in_array($retrieve_condition, ['never', 'omitted'])):
                ...
                case ( ! $coordinates_included && in_array($retrieve_condition, ['omitted', 'always'])):
