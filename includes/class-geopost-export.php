...
class GeoPostExport {
  ...
  public static function admin_enqueue_scripts() {
    if ( isset($_GET['page']) && $_GET['page'] === 'geopost_settings'
      ...
      wp_enqueue_script( 'download-js', plugins_url('lib/download.min.js', GeoPost::$plugin_file ), array(), '4.2', true );
    }
  }
  ...
  public static function ajax_export() {
    ...
    foreach($posts as $post) {
      ...
      // Write to file
      if ( !fputcsv($out, $line) ) {
        wp_send_json_error('Error: There was a problem adding the following line to the CSV file. Please check for issues: ' . implode(', ', $line));
      }
    }
